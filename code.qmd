...
This script implements a **nested, group-aware cross-validated neural network training pipeline** for regression using Keras. To prevent data leakage across repeated observations of the same player, all cross-validation splits are **grouped by `player_api_id`**, ensuring that each player appears in only one fold at a time. A 5-fold **outer cross-validation loop** is used to obtain unbiased estimates of generalization performance, while an **inner validation split (20% of players within each fold)** is employed for hyperparameter selection. Within each fold, features are standardized using parameters learned exclusively from the inner training set. Model selection is performed via a grid search over network architecture (three hidden layers), dropout rates, and learning-rate annealing factors. Each model is trained with **L2 regularization, batch normalization, dropout, early stopping, and adaptive learning-rate reduction/annealing** to improve convergence and reduce overfitting. The best-performing hyperparameter configuration (based on validation MSE) is then refit on the full outer training fold and evaluated on the held-out test fold. Trained models, optimal hyperparameters, loss trajectories, and fold-specific performance metrics (MSE and MAE) are saved to disk to ensure reproducibility and facilitate post-hoc analysis.
...
# Initialize storage for outer CV results, as well as best hyperparameters and best loss logs per fold
...
# Build, compile, and train 3-hidden-layer neural network with regularization, early stopping, learning-rate annealing, and custom loss logging; returns fitted model and validation loss
build_and_fit <- function(X_train, y_train,
  ...
  ## Return the trained model, training history, and best validation loss
  list(
    model = model,
    history = history,
...
for (i in start_i:length(outer_folds)) {
  ...
  for (g in seq_len(nrow(grid))) {
    ...
    #### Tracks minimum validation loss for each grid combination
  ...
  ## Restrict max_epoch based on validation early-stopping,
  ## allow for some extra convergence
  max_epoch<-ceiling(length(best_fit$history$metrics$val_loss)*1.2)
  
  ## Refit best model of grid search with full fold training set
...
## Save best parmeters and loss descent of optimal hyperparameter combination per fold
