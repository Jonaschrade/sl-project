...
for (i in start_i:length(outer_folds)) {
  ...
  for (g in seq_len(nrow(grid))) {
    ...
    #### Log validation loss per fold, per grid, per epoch
    loss_log[[paste(i, g, sep = "_")]] <-
      tibble(
        outer = i,
        grid  = g,
        epoch = seq_along(fit$history$metrics$loss),
        loss = fit$history$metrics$loss,
        val_loss = fit$history$metrics$val_loss
      )
    
    #### Checkpoint loss log
    saveRDS(loss_log, file = "models_nn/loss_log_checkpoint.rds")
...
best_params_df <- bind_rows(best_params, .id = "Fold")
