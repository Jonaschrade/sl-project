...
While the neural network appears to exhibit greater capacity to model complex relationships between players’ physical and cognitive attributes, age, and potential, consistent with our expectation that the omission of positional interactions would disadvantage the regression model more than the network, this expectation is ultimately not confirmed. A qualitative comparison of the ten players with the largest average overprediction and underprediction errors across both models does not reveal systematic differences by position, as no position predominates in either group. However, more granular positional information at the player year level would be required to enable a more thorough quantitative assessment of such underlying complexity. Nevertheless, this is consistent with the notion that although neural networks may flexibly capture complex and potentially high dimensional structures, such patterns may neither manifest in interpretable group level regularities nor be readily identifiable by the researcher, let alone explicitly specified in a parametric model.

Furthermore, the study design does not permit predictions of players’ career development, as both the modeling approach and data splitting are strictly cross sectional. Consequently, no inferences can be made about individual career trajectories or whether a player ultimately realizes their true potential. Each player is treated solely as a unit of observation, with potential assessments derived from pooled historical data across multiple generations of players. This cross sectional inference further assumes temporal stability in the relationship between player features and potential, both in real world development processes and in FIFA’s underlying assessment methodology across time.
