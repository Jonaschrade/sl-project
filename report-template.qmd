...
Within each training fold, the regularization parameter $\lambda$ is selected via internal cross-validation over a grid of candidate values. The model is then refit using the selected $\lambda$, and predictive accuracy is assessed using MSE and MAE on training and held-out test data.
